...
class Body(list):
    ...
    def create(cls, data):
        if not isinstance(data, (list,tuple)):
            raise ValueError('Invalid BODY input.')
        elif isinstance(data[0], tuple):
            return BodyTypeMPart(data)
        elif (isinstance(data[0], six.string_types)
              and isinstance(data[1], six.string_types)):
            return BodyType1Part(data)
        else:
            raise ValueError('Invalid BODY input.')

class BodyType1Part(Body):
    def __init__(self,data):
        self.extend(data)
        if (data[0].lower() == 'message' and data[1].lower() == 'rfc822'):
            self.type = 'body-type-msg'
        elif (data[0].lower() == 'text'):
            self.type = 'body-type-text'
        else:
            self.type = 'body-type-basic'
        content_type = "{}/{}".format(data[0].lower(),data[1].lower())
        self.set_content_type(content_type)

class BodyTypeMPart(Body):
    def __init__(self,data):
        self.type = 'body-type-mpart'
        for i,part in enumerate(data):
            if isinstance(part, six.string_types):
                break
        newparts = list(map(Body.create,data[:i]))
        newparts.extend(data[i:])
        self.extend(newparts)
        content_type = "multipart/{}".format(part.lower())
        self.set_content_type(content_type)
...
class BodyData(tuple):
    ...
    def create(cls, response):
        ...
        if (cls.nest and isinstance(response[0], tuple)):
    ...
    def get_section(self, partnumber):
        """
        Uses IMAP section part numbers to get elements from the BodyData tuple.

        In [510]: b.get_section('1.2.5')
        Out[510]: (u'TEXT', u'HTML', (u'CHARSET', u'us-ascii'), None, None, u'7BIT', 283, 0)
        
        In [511]: c.fetch(767,['BODY[1.2.5.MIME]'])
        Out[511]: defaultdict(<type 'dict'>, {767: {u'BODY[1.2.5.MIME]': u'Content-Transfer-Encoding: 7bit\r\nContent-Type: text/html;\r\n\tcharset=us-ascii\r\n\r\n', u'SEQ': 725}})
        """
        ...
        for index,part in enumerate(parts):
            part = int(part) - 1
            if (part<0):
                raise IndexError('Part number {} is too small'
                                 .format('.'.join(parts[:index+1])))
            if (part>=len(response)):
                raise IndexError('Part number {} is too large'
                                 .format('.'.join(parts[:index+1])))
